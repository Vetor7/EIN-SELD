...
class UserDataset(Dataset):
    ...
    def __init__(self, args, cfg, dataset, dataset_type='train', overlap=''):
        ...
        main_data_dir = Path(cfg['hdf5_dir']).joinpath('data')
        ...
        main_meta_dir = Path(cfg['hdf5_dir']).joinpath('meta')
...
class PinMemCustomBatch:
    def __init__(self, batch_dict):
        ...
        self.batch_out_dict = {
            ...
            'ov': batch_ov,
            'waveform': torch.tensor(np.array(batch_x), dtype=torch.float32),
            'sed_label': torch.tensor(np.array(batch_sed_label), dtype=torch.float32),
            'doa_label': torch.tensor(np.array(batch_doa_label), dtype=torch.float32),
        }
